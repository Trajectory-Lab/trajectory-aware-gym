...
| IT-03 | Tools + Config | Task model routing selects provider model and train/eval temperature for all 7 variants | Yes | `tests/integration/test_system_pipeline.py::test_task_model_routing_and_temperature` | Parametrized across Ollama (qwen3:1.7b, 4b), SageMaker (qwen3-sagemaker:1.7b, 4b), and Bedrock (llama:1b, 3b, 8b) |
