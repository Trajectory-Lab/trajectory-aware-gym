...
class TestExecute:
    def test_execute_calls_tool_and_logs(self, runtime, tmp_path):
        ...
        with patch.object(runtime, "_run_sync", side_effect=[mock_tool, mock_result]) as mock_sync:
    ...
    def test_execute_defaults_arguments_to_empty_dict(self, runtime):
        ...
        with patch.object(runtime, "_run_sync", side_effect=[mock_tool, {"ok": True}]):
    ...
    def test_execute_normalizes_non_dict_result(self, runtime):
        ...
        with patch.object(runtime, "_run_sync", side_effect=[mock_tool, "raw"]):
    ...
    def test_execute_returns_error_dict_on_unknown_tool(self, runtime, tmp_path):
        with patch.object(runtime, "_run_sync", side_effect=KeyError("no_such_tool")):
    ...
    def test_execute_returns_error_on_value_error(self, runtime):
        with patch.object(runtime, "_run_sync", side_effect=ValueError("invalid tool")):
...
class TestRunSyncWithLoop:
    ...
    def test_thread_timeout_raises(self, runtime):
        async def inner():
            ...
            return runtime._run_sync(slow_coro())
...
class TestListSchemas:
    ...
    def test_returns_tool_schemas(self, runtime):
        ...
        with patch.object(runtime, "_run_sync", return_value={"tool_a": tool_a, "tool_b": tool_b}):
    ...
    def test_single_tool(self, runtime):
        ...
        with patch.object(runtime, "_run_sync", return_value={"only": tool}):
            schemas = runtime.list_schemas()
